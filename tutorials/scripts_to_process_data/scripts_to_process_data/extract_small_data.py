...
# Set up logging
logging.basicConfig(filename = 'datasubset.log',
                    level=logging.INFO, 
                    format='%(asctime)s - %(levelname)s - %(message)s')

# Extract specific strains from the phenotypic data
def extract_phenotypic_data(input_file, selected_strains):
    '''
    This function extracts specific strains from the phenotypic data and output to a new file.
    
    selected_strains: a tab-delimited file with the first row as the strain names to extract
    ...
    '''
    # Read the selected strain names
    with open(selected_strains, 'r') as f:
        strain_rows = [line.strip().split('\t') for line in f.readlines()]
        strains = strain_rows[0]

    # Extract phenotypes and output to a new file
...
def bcftool_extract_snp(input_vcf, output_vcf, selected_strains):
    '''
    This function extracts snps from a vcf file using bcftools.

    selected_strains: a list of strains to extract
    ...
    logging.info("Making sure vcf contains only biallelic SNPs...")
    logging.info("Running bcftools with command: %s", command)
    subprocess.run(command, shell=True)
    logging.info("SNP extraction completed. Output file: %s", output_vcf)
...
def filter_snp_data(input_snp_vcf, output_vcf, missing_threshold=0.95, maf_threshold=0.01):
    '''
    This function filters the snp data based on the number of missing values and minor allele frequency.
    Outputs a gzipped VCF file.
    '''
    ...
    logging.info(f"Filtering SNPs based on missing data proportion >= {missing_threshold} and MAF <= {maf_threshold}...")
    ...
    # Report statistics
    logging.info("")
    logging.info(f"Number of SNPs with missing values over {100-missing_threshold * 100}%: {num_missing}")
    logging.info(f"Number of SNPs with minor allele frequency smaller than {maf_threshold}: {num_maf_fail}")
    logging.info("")
    # Close the files
    vcf_in.close()
    vcf_out.close()
    logging.info(f"Filtered SNPs written to {output_vcf_gz}")
...
def report_snp_data_statistics(input_snp_vcf):
    ...
    logging.info("")
    logging.info(f"Number of SNPs: {num_snps}")
    logging.info(f"Number of strains: {num_strains}")
    logging.info("")
...
def main():
    ...
    parser.add_argument("-s","--selected_strains", help="A tab-delimited file with the first row as the strain names to extract.")
    args = parser.parse_args()

    phenotype_file = os.path.join(os.getcwd(), args.phenotypic_data)
    vcf_file = os.path.join(os.getcwd(), args.genetic_variant_data)
    strains = args.selected_strains
    ...
    logging.info("Summary statistics before filtering:")
    report_snp_data_statistics(extract_file)

    # Filter snp data
    filtered_out = f"{output_prefix}.snp.subset.filtered.vcf"
    filter_snp_data(extract_file, filtered_out, missing_threshold=0.95, maf_threshold=0.01)
    
    # Report snp data statistics
    logging.info("Summary statistics after filtering:")
